/*
  # Complete Rewards System Schema

  1. New Columns for Users Table
    - `ida_balance` (numeric) - User's IDA token balance
    - `total_earned` (numeric) - Total IDA tokens earned
  ...
  3. Security
    ...
    - Fix referral_stats INSERT policy issue
...
CREATE POLICY "Service role full access user_tasks"
  ON user_tasks
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role full access referral_stats"
  ON referral_stats
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Service role full access referral_rewards"
  ON referral_rewards
  FOR ALL
  TO service_role
  USING (true)
...
CREATE POLICY "Users can read own transactions"
  ...
  USING (EXISTS (
    ...
    AND (users.wallet_address = transaction_history.from_address OR users.wallet_address = transaction_history.to_address)
  ));

CREATE POLICY "Service role full access transaction_history"
  ON transaction_history
  FOR ALL
  TO service_role
  USING (true)
...
INSERT INTO tasks (title, description, reward_amount, xp_reward, task_type, is_active) VALUES
  ('Connect Wallet', 'Connect your MultiversX wallet to the platform', 100, 10, 'wallet_connect', true),
  ('Complete Profile', 'Fill out your complete profile information', 200, 20, 'profile_complete', true),
  ('Create First Gig', 'Create your first service offering', 500, 50, 'gig_create', true),
  ('Complete First Order', 'Successfully complete your first order', 1000, 100, 'order_complete', true),
  ('Reach Level 5', 'Reach level 5 in the mining system', 2000, 200, 'mining_level', true)
