...
export const Chat = () => {
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div className="fixed bottom-0 left-0 right-0 md:inset-auto md:bottom-6 md:left-6 z-[1000002] md:w-96 h-[90vh] max-h-[700px] md:h-[600px] bg-gray-800 md:rounded-xl rounded-t-3xl shadow-2xl flex flex-col overflow-hidden border-2 border-purple-500 animate-slide-up">
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-2 md:p-4 space-y-2 md:space-y-3 bg-gray-900">
        {messages.length === 0 ? (
          <div className="text-center text-gray-500 py-8">
            <MessageCircle size={40} className="mx-auto mb-2 opacity-50 md:w-12 md:h-12 text-white" />
            <p className="text-sm md:text-base text-white">No messages yet. Start the conversation!</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              ...
              >
                <div className="flex-shrink-0 w-7 h-7 md:w-8 md:h-8 rounded-full bg-purple-600 flex items-center justify-center text-base md:text-lg">
                  {msg.avatarUrl}
                </div>
                <div className={`flex-1 max-w-[75%] md:max-w-xs ${isOwnMessage ? 'items-end' : 'items-start'} flex flex-col`}>
                  ...
                  >
                    {isFoodTransfer ? (
                      ...
                      <div className="flex items-center gap-1.5">
                        <Gift size={14} className="text-purple-400 flex-shrink-0 md:w-4 md:h-4" />
                  ...
                  {!isOwnMessage && !isTransfer && (
                    <div className="flex items-center gap-2 mt-0.5 md:mt-1">
                      <button
                        ...
                        className="text-xs text-yellow-400 hover:text-yellow-300 active:text-yellow-500"
                      >
                        Send Food 🍖
                      </button>
                      ...
                      <button
                        ...
                        className="text-xs text-purple-400 hover:text-purple-300 active:text-purple-500"
                      >
                        Send Tickets 🎫
                      </button>
                    </div>
      ...
      {showSendFood && selectedRecipient && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-3 md:p-4 z-10">
          <div className="bg-gray-800 rounded-xl p-4 md:p-6 max-w-sm w-full space-y-3 md:space-y-4 border-2 border-purple-500">
            <div className="flex items-center justify-between gap-2">
              <h4 className="font-bold text-sm md:text-base text-white truncate">Send Food to {selectedRecipient.username}</h4>
              <button onClick={() => setShowSendFood(false)} className="text-gray-400 hover:text-white flex-shrink-0 p-1">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-2">
              <label className="text-xs md:text-sm font-medium text-white">Amount (🍖 Food)</label>
              <input
                type="number"
                value={foodAmount}
                onChange={(e) => setFoodAmount(e.target.value)}
                placeholder="Enter amount..."
                className="w-full px-3 py-2 text-sm md:text-base bg-gray-700 text-white border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                min="1"
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 bg-gray-700 text-white hover:bg-gray-600 active:bg-gray-500 px-4 py-2 rounded-lg font-medium text-sm transition-colors"
              >
                Cancel
              </button>
              <button
      ...
      {showSendTickets && selectedRecipient && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-3 md:p-4 z-10">
          <div className="bg-gray-800 rounded-xl p-4 md:p-6 max-w-sm w-full space-y-3 md:space-y-4 border-2 border-purple-500">
            <div className="flex items-center justify-between gap-2">
              <h4 className="font-bold text-sm md:text-base text-white truncate">Send Tickets to {selectedRecipient.username}</h4>
              <button onClick={() => setShowSendTickets(false)} className="text-gray-400 hover:text-white flex-shrink-0 p-1">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-2">
              <label className="text-xs md:text-sm font-medium text-white">Amount (🎫 Tickets)</label>
              <input
                type="number"
                value={ticketAmount}
      ...
      {/* Emoji Picker */}
      {showEmojiPicker && (
        <div className="absolute bottom-16 left-2 right-2 md:bottom-20 md:left-4 md:right-auto bg-gray-800 rounded-lg shadow-xl p-3 md:p-4 border-2 border-purple-500 z-20 max-w-sm">
          <div className="flex items-center justify-between mb-3">
            <h4 className="text-sm font-bold text-white">Pick an Emoji</h4>
            <button onClick={() => setShowEmojiPicker(false)} className="text-gray-400 hover:text-white p-1">
              <X size={18} />
            </button>
          </div>
          <div className="grid grid-cols-6 md:grid-cols-7 gap-1.5 md:gap-2 max-h-40 md:max-h-48 overflow-y-auto">
            {EMOJI_LIST.map((emoji, idx) => (
              <button
                ...
                className="text-xl md:text-2xl hover:scale-110 active:scale-95 transition-transform p-1.5 md:p-2 hover:bg-gray-700 rounded"
              >
                {emoji}
              </button>
            ))}
      ...
      {/* GIF Picker */}
      {showGifPicker && (
        <div className="absolute bottom-16 left-2 right-2 md:bottom-20 md:left-4 md:right-auto bg-gray-800 rounded-lg shadow-xl p-3 md:p-4 border-2 border-purple-500 z-20 md:w-80">
          <div className="flex items-center justify-between mb-3">
          ...
          <div className="mb-3">
            <div className="relative mb-2">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 text-sm bg-gray-700 text-white border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>
