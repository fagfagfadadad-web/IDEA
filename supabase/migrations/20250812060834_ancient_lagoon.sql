/*
  ...
  1. New Tables
    - `tasks` - System-wide tasks that users can complete for rewards
      - `id` (uuid, primary key)
    ...
    - `user_tasks` - Individual user progress on tasks
      - `id` (uuid, primary key)
  ...
  2. Table Updates
    - Add missing columns to `users` table:
      - `ida_balance` (numeric, IDA token balance)
...
-- Create user_tasks table with proper foreign key relationship
...
-- RLS Policies for user_tasks table (users can only see their own tasks)
...
CREATE POLICY "Service role can manage transaction history"
  ON transaction_history
  FOR ALL
  TO service_role
  USING (true)
...
-- Fix RLS policy for referral_stats to allow authenticated users to insert their own data
...
ALTER TABLE tasks ADD CONSTRAINT tasks_task_type_check CHECK (task_type IN ('manual', 'wallet_connect', 'transaction', 'mining_level', 'referral', 'social_follow', 'social_post', 'social_retweet', 'social_like', 'external_link'));
ALTER TABLE tasks ADD CONSTRAINT tasks_proof_type_check CHECK (proof_required_type IN ('none', 'file', 'url', 'text'));

ALTER TABLE user_tasks ADD CONSTRAINT user_tasks_status_check CHECK (status IN ('available', 'in_progress', 'completed', 'claimed'));

ALTER TABLE transaction_history ADD CONSTRAINT transaction_history_amount_check CHECK (amount >= 0);
ALTER TABLE transaction_history ADD CONSTRAINT transaction_history_type_check CHECK (transaction_type IN ('send', 'receive', 'reward', 'referral'));
ALTER TABLE transaction_history ADD CONSTRAINT transaction_history_status_check CHECK (status IN ('pending', 'success', 'failed'));
...
INSERT INTO tasks (title, description, reward_amount, xp_reward, task_type, proof_required_type, is_active) VALUES
  ...
  ('Refer a Friend', 'Invite someone to join IDEA using your referral link', 1500, 75, 'referral', 'none', true),
  ('Complete First Order', 'Successfully complete your first order as a provider', 3000, 150, 'manual', 'none', true),
  ('Reach Level 5', 'Accumulate enough XP to reach level 5', 2500, 0, 'mining_level', 'none', true)
